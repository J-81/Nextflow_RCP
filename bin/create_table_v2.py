...
def _parse_args():
    ...
    parser.add_argument('--accession', metavar='GLDS-194', required=True,
                        help='GeneLab dataset accession number')
    parser.add_argument('--isa-zip', metavar='ISA.ZIP', required=True,
...
def open_files(success, isa_zip_path):
    ...
    return [success, sample_file, investigation_file, file_name_s, file_name_i, metadata_directory]
...
def s_file_parse(s_file):
    ...
    for idx, title in enumerate(s_file.headers):
        ...
        if "organism" in title.lower() or "strain" in title.lower() or "cell line" in title.lower() or "ecotype" in title.lower() or "cultivar" in title.lower() :
        ...
        if "hardware" in title.lower() or "material type" in title.lower() or "age" in title.lower() or "genotype" in title.lower() or "temperature" in title.lower() or "preservation method" in title.lower():
    ...
    if len(s_file.characteristics_change_idx)>0:
        for i in range(len(s_file.characteristics_change_idx)):
            s_file.final_characteristics_idx.append(s_file.characteristics_change_idx[i])
            for element in s_file.units_idx:
    ...
    if "arabidopsis thaliana" in organism_column or "brassica rapa" in organism_column or "ceratopteris richardii" in organism_column or "chlamydomonas reinhardtii" in organism_column or "eruca vesicaria" in organism_column:
        s_file.organism_type = "plant"

    column_index = []
    if s_file.organism_type == "plant":
        column_index.extend(s_file.sample_name_idx + s_file.original_sample_name_idx + s_file.final_characteristics_idx + s_file.final_plant_params_idx + s_file.final_parameters_idx + s_file.final_factors_idx)
    else:
        column_index.extend(s_file.sample_name_idx + s_file.original_sample_name_idx + s_file.final_characteristics_idx + s_file.final_parameters_idx + s_file.final_factors_idx)
...
def a_file_parse(a_file, i_file, metadata_directory):
    ...
    for i in range(len(i_file.assay_tech_types)):
        ...
        if file_found == 0:
            print("ERROR: Could not find an assay file for the device type.\n Make sure the assay file name contains the device type or contact Ana to add it.")
            sys.exit()

        file_name_a = metadata_directory+"/"+filename
        if file_name_a == "":
            print("ERROR: No assay file found for device type "+ device_type +". Please make sure the metadata files are complete and named correctly.")
            print("The device type must be included in the asssay file name.")
            sys.exit()
        try:
            f1 = open(os.path.join(metadata_directory, file_name_a), "r", encoding='windows-1252')
            assay_file = f1.read()
            f1.close()
        ...
        for i, line in enumerate(assay_file.split("\n")):
            ...
            for j, column in enumerate(line.split("\t")):
                if i == 0:
                    if "raw data file" in column.lower() or "array data file" in column.lower() or "raw spectral data file" in column.lower():
                        flag += 1
                        if flag == 1:
                            idx = j
                    if "sample name" in column.lower() and "comment" not in column.lower():
                        sample_name_idx = j
                else:
                    if j == idx:
...
def create_table(i_file, s_file, a_file, data_columns, column_index, glds_num):
    ...
    extra_columns = 0
    if s_file.organism_type == "plant":
        required_fields = ["Hardware", "Material Type", "Age", "Ecotype", "Genotype", "Time", " Light", "Temperature", "Treatment type", "Dose", "Preservation method"]
    ...
    for i in range(0, s_file.num_lines -1 ):
        ...
        for j in range(len(header)+len(unit_idx)):
            ...
            if j > (len(i_file.assay_tech_types) + 2) and j < (len(header) + len(unit_idx) - extra_columns - 1):
                if j-(len(i_file.assay_tech_types)+2) not in unit_idx:
                    ...
                    if j+1-(len(i_file.assay_tech_types)+2) in unit_idx:
                        line += data_columns[j-(len(i_file.assay_tech_types)+2)][i].strip('"') + " " + data_columns[j+1-(len(i_file.assay_tech_types)+2)][i].strip('"') + "\t"
                    #if there is no unit, just add value
                    else:
                        line += data_columns[j-(len(i_file.assay_tech_types)+2)][i].strip('"') + "\t"
